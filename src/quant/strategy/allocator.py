...
def risk_on_candidates(
    ...
    """
    Return sorted candidates (risk_adj score) after absolute filters for Risk ON.
    Index: ticker, Value: score (higher is better)
    """
...
def choose_top_assets(
    ...
    Choose top assets under absolute filters and return:
      ...
      - ranked_top: list of (ticker, score) for the top_k candidates
    """
    top_n = int(params["selection"]["top_n"])

    candidates = risk_on_candidates(dt, prices_row, feats, universe_cfg, params)
...
def pick_gear_for_asset(
    ...
    if lev.get("use_vol_target", False):
        ...
        cut_2x = float(lev.get("gear_cut_2x", 1.5))
        cut_3x = float(lev.get("gear_cut_3x", 2.5))
